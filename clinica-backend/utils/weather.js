...
const apiUtils = {
  // Buscar previsão do tempo do OpenWeather
  async getPrevisaoTempo(cidade, dataAgendamento) {
    try {
      ...
      const geoResponse = await axios.get(
        `http://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(cidade)},BR&limit=1&appid=${API_KEY}`
      );
      ...
      const vaiChover = previsaoMaisProxima.weather.some(cond => 
